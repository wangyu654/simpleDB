...
func TestReElection2A(t *testing.T) {
	...
	cfg.checkOneLeader()
	fmt.Println("test 1: pass at", time.Now().Unix()-start)

	// if the old leader rejoins, that shouldn't
	// disturb the new leader.
	cfg.connect(leader1)
	leader2 := cfg.checkOneLeader()
	fmt.Println("test 2: pass at", time.Now().Unix()-start)
	...
	time.Sleep(2 * RaftElectionTimeout)
	cfg.checkNoLeader()
	fmt.Println("test 3: pass at", time.Now().Unix()-start)

	// if a quorum arises, it should elect a leader.
	cfg.connect((leader2 + 1) % servers)
	cfg.checkOneLeader()
	fmt.Println("test 4: pass at", time.Now().Unix()-start)

	// re-join of last node shouldn't prevent leader from existing.
	cfg.connect(leader2)
	cfg.checkOneLeader()
	fmt.Println("test 5: pass at", time.Now().Unix()-start)

	cfg.end()
}

func TestLabB(t *testing.T) {
	TestBasicAgree2B(t)
	TestRPCBytes2B(t)
	TestFailAgree2B(t)
	TestFailNoAgree2B(t)
...
// 			}
// 		}
// 		log.Println("roll:", iters, "next net condition:", cfg.connected, "nup:", nup, "consuming:", (time.Now().Second() - start), "s", "spend:", time.Now().Second()-start1, "s")
// 	}
